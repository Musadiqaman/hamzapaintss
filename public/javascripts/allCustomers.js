document.addEventListener("DOMContentLoaded", () => {
    ...
    // =============================================
    // 2. PAGINATION UI UPDATE (FIXED STATE)
    // =============================================
    function updatePaginationUI(pagination) {
        ...
        // CRITICAL FIX: Update global states & DOM elements datasets
    ...
    const runFilter = async (customParams = null) => {
        ...
        try {
            const res  = await fetch(`/customers/all?${queryString}`, {
                headers: {
            ...
            if (data.success) {
                ...
                if (statsPs.length >= 4) {
                    statsPs[0].innerText = data.stats.totalCustomers;
                    ...
                    // FIXED: Removed .toDate() that was crashing application
                        ...
                        html += `
                            <tr>
                                ...
                                <td class="action-buttons">
                                    <button id="view" >
                                        <a href="/customers/view/${c._id}" style="text-decoration: none; color: inherit;">View</a>
                                    </button>
                                    ${data.role === "admin"
  ...
  function attachDeleteListeners() {
    document.querySelectorAll(".delete-btn").forEach(btn => {
        btn.onclick = async (e) => {
            ...
            try {
                const res  = await fetch(`/customers/delete-customer/${id}`, { method: "DELETE" });
                const data = await res.json();
                if (data.success) {
                    alert("✅ " + data.message);
                    runFilter();
    ...
    // =============================================
    // 7. PAGINATION EVENTS (RE-VERIFIED LOGIC)
    // =============================================
    if (prevBtn) {
        prevBtn.onclick = () => {
            if (currentPage > 1) { 
