document.addEventListener("DOMContentLoaded", function() {
    ...
    var agentId   = pathParts[pathParts.length - 1];

    var formatRs = function(num) { return Number(num).toFixed(2); };
    ...
    var fetchData = async function() {
        if (!agentId || agentId.length < 15) return;
        ...
        try {
            var response = await fetch("/agents/view/" + agentId + "?" + params, {
            ...
            if (data.success) {
                ...
                if (!data.agent.items || data.agent.items.length === 0) {
                    ...
                    data.agent.items.forEach(function(i) {
                        ...
                        var status   = i.paidAmount >= i.percentageAmount ? 'Paid' : (i.paidAmount > 0 ? 'Partially' : 'Unpaid');
                        var leftAmt  = formatRs(Number(i.percentageAmount) - Number(i.paidAmount));
    ...
    function rebindButtons() {
        ...
        document.querySelectorAll(".submit-pay-btn").forEach(function(btn) {
            btn.onclick = async function() {
                ...
                btn.innerText = "...";
                try {
                    var res    = await fetch("/agents/pay-item/" + id, {
                        method: "POST",
        ...
        document.querySelectorAll(".delete-btn").forEach(function(btn) {
            btn.onclick = async function() {
                ...
                try {
                    var res    = await fetch("/agents/delete-item/" + btn.dataset.id, { method: "DELETE" });
                    var result = await res.json();
                    if (result.success) { alert(result.message); fetchData(); }
    ...
    if (saveProfileBtn) {
        saveProfileBtn.onclick = async function() {
            ...
            try {
                var res    = await fetch("/agents/update/" + agentId, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ name: name, phone: phone })
                });
                var result = await res.json();
                if (result.success) { alert("Agent Profile Updated!"); location.reload(); }
    ...
    if (viewHistoryBtn) {
        viewHistoryBtn.onclick = async function() {
            ...
            try {
                var res  = await fetch("/agents/payment-history/" + agentId);
                    ...
                    groups.forEach(function(group) {
                        ...
                        if (group.length === 1) {
                            ...
                            html += '<tr>'
                                ...
                                + '<td style="padding:10px; border-bottom:1px solid #eee; font-weight:bold; color:green;">Rs ' + Number(h.amountPaid).toFixed(2) + '</td>'
                                + '</tr>';
                        } else {
                            ...
                            group.forEach(function(h, idx) {
                                ...
                                html += '<tr style="background:#f8fbff;">'
                                    ...
                                    + '<td style="padding:6px 12px; font-weight:bold; color:green; font-size:13px; border-bottom:' + border + ';">'
    ...
    if (submitCollective) {
        submitCollective.onclick = async function() {
            ...
            try {
                var res  = await fetch("/agents/collective-pay/" + agentId, {
                    method:  "POST",
                    headers: { "Content-Type": "application/json" },
